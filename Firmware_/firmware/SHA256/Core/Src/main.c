...
int main(void)
{
  ...
  double exec_time_us=exec_time_sec*1000000;

  double cycles_per_byte=avg_cycles/(float)MESSAGE_SIZE_BYTES;


  double throughput=((float)MESSAGE_SIZE_BYTES/exec_time_sec)/1024.0;

  double energy_operation=SUPPLY_VOLTAGE*ACTIVE_CURRENT*exec_time_sec*1000000;

  double energy_per_byte=energy_operation/(float)MESSAGE_SIZE_BYTES;


  printf("========= Algorithm: SHA-256 =========\r\n");

  printf("Clock Frequency : %.0f MH\r\n",CPU_FREQ_MHZ);
